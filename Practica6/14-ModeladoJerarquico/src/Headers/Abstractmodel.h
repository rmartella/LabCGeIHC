...
public:
	...
	void setUVS(std::vector<glm::vec2> uvs) {
		assert(uvs.size() == vertexArray.size());
		for (int i = 0; i < vertexArray.size(); i++)
			vertexArray[i].m_tex = uvs[i];
		update();
	}
	...
	void setColor(glm::vec3 color, std::string uniformColorId = "color") {
		this->color = color;
		shader_ptr->turnOn();
		GLint locColor = shader_ptr->getUniformLocation(uniformColorId);
